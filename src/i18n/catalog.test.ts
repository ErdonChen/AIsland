...
test("includes the exact clipboard history copy in both locales", () => {
  const approved: Record<string, readonly [string, string]> = {
    ...
    "clipboard.confirm.delete": ["删除这条 AIceLand 剪贴板记录？", "Delete this AIceLand clipboard item?"],
    "clipboard.confirm.clear": ["清空所选范围内的 AIceLand 剪贴板历史？", "Clear AIceLand clipboard history in the selected scope?"],
...
test("includes the fixed notification center catalog table exactly in both locales", () => {
  const exact = {
    ...
    "notifications.origin.aiceland": ["AIceLand", "AIceLand"],
    ...
    "notifications.deleteConfirm": ["仅从 AIceLand 通知中心移除此条记录？Windows 原通知不会被修改。", "Remove this item only from AIceLand history? The original Windows notification will not be changed."],
    "notifications.clearConfirm": ["仅清空 AIceLand 保存的通知记录？Windows 原通知和提醒历史不会被删除。", "Clear only notification records saved by AIceLand? Original Windows notifications and reminder history will not be deleted."],
...
test("includes every approved built-in agent and reminder literal exactly", () => {
  const approved = {
    ...
    "agents.integration.install": ["安装集成", "Install integration"], "agents.integration.repair": ["修复集成", "Repair integration"], "agents.integration.uninstall": ["卸载集成", "Uninstall integration"], "agents.integration.installed": ["已安装", "Installed"], "agents.integration.needsRepair": ["需要修复", "Needs repair"], "agents.integration.unsupported": ["此环境不受支持", "This environment is not supported"], "agents.integration.confirmTitle": ["仅移除 AIceLand 集成", "Remove only the AIceLand integration"], "agents.integration.confirmBody": ["将备份当前配置，并仅移除 AIceLand 管理的条目。", "The current configuration will be backed up, and only AIceLand-managed entries will be removed."],
