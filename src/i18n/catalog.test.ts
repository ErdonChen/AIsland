...
test("matches the complete authoritative daily-note table exactly in both locales", () => {
  const copy = {
    ...
    "notes.calendar.show": ["显示日历", "Show calendar"],
    "notes.calendar.hide": ["收起日历", "Hide calendar"],
    "notes.calendar.previous": ["上个月", "Previous month"],
    "notes.calendar.next": ["下个月", "Next month"],
    "notes.calendar.today": ["今天", "Today"],
    "notes.calendar.hasContent": ["有内容", "has content"],
    "notes.calendar.error": ["无法加载日历内容标记", "Unable to load calendar content markers"],
    "notes.recording.title": ["录音", "Recordings"],
    "notes.recording.start": ["开始录音", "Start recording"],
    "notes.recording.stop": ["停止录音", "Stop recording"],
    "notes.recording.active": ["正在录音", "Recording"],
    "notes.recording.empty": ["当天还没有录音", "No recordings for this day"],
    "notes.recording.load": ["加载录音", "Load recording"],
    "notes.recording.delete": ["删除录音", "Delete recording"],
    "notes.recording.deleteConfirm": ["删除这段本地录音？", "Delete this local recording?"],
    "notes.recording.item": ["录音 {count}", "Recording {count}"],
    "notes.recording.error": ["录音操作失败，请检查麦克风权限后重试", "Recording failed. Check microphone permission and try again."],
    "notes.recording.error.permission": ["无法使用麦克风，请检查系统权限后重试", "Unable to use the microphone. Check system permission and try again."],
    "notes.recording.error.encoding": ["录音编码失败，本次录音未保存", "Recording encoding failed. This recording was not saved."],
    "notes.recording.error.storage": ["录音未保存，请重试", "The recording was not saved. Try again."],
    "notes.recording.error.unsupported": ["当前系统录音格式不受支持", "This system recording format is not supported."],
  } as const;
