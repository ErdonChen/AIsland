...
- Markdown 每日笔记：按日期保存，支持搜索、复制、导出和打开笔记目录；可展开月历查看带有文字或录音的日期，并手动调用系统默认麦克风保存多段本地录音。
- 剪贴板历史：管理文字和图片记录，支持搜索、筛选、置顶、复制与删除。
- 系统监控：查看 CPU、内存、磁盘、网络和 GPU 指标。
- 托盘与启动：窗口可收进系统托盘，并可在设置中管理开机启动、语言、缩放和外观；显示与外观提供六种背景色圆形预览、独立玻璃透明度，以及弹性、平滑和快速三种真实展开动效预览。

每日笔记的文字与录音相互独立：删除当天文字不会删除录音，没有文字但有录音的日期仍会显示在月历内容标记中。切换日期或离开每日笔记时，AIsland 会先完成当前保存与录音收尾；失败时会留在原页面以便重试。
...
当前版本：[`preview-v0.1.0.5`](https://github.com/ErdonChen/AIsland/releases/tag/preview-v0.1.0.5)

- [下载 Windows 11 x64 NSIS 安装包](https://github.com/ErdonChen/AIsland/releases/download/preview-v0.1.0.5/AIsland_0.1.0_x64-setup.exe)
- [查看 SHA256SUMS.txt](https://github.com/ErdonChen/AIsland/releases/download/preview-v0.1.0.5/SHA256SUMS.txt)
